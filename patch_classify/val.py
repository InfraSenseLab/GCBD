...
def box2grid(p_grid, box):
    B, C, H, W = p_grid.shape
    grid = torch.zeros_like(p_grid, device=p_grid.device)
    b = (box[:, 0]).long()
    c = (-box[:, 1] - 1).long()
    h = (box[:, 3] * H).long()
    w = (box[:, 2] * W).long()
...
def grid2box(prediction, threshold, stride):  # [bs,3,gy,gx]-->[bs,nc,6]
...
def run(
    ...
    if not training:
        ...
        pad, rect = (0.0, False) if task == 'speed' else (1.5, pt)  # square inference for benchmarks
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # NMS
        targets, targets_grid = targets[targets[:, 1] >= 0], targets[targets[:, 1] < 0]
        ...
        with dt[2]:
            preds = non_max_suppression(preds,
                                        ...
                                        max_det=max_det)
        # Grid Metrics
        target_cls = targets_grid[:, 1]
        labels_grid = box2grid(preds_grid, targets_grid)  # b,nc_grid,x,y
        pred_index = preds_grid > conf_thres
        pred_cls = torch.zeros_like(preds_grid)
        for i in range(pred_cls.shape[1]):
            pred_cls[:, i, ...] = -i - 1
        correct = labels_grid[pred_index].reshape(-1, 1).bool()
        conf = preds_grid[pred_index]
        pred_cls = pred_cls[pred_index]
        stats_grid.append((correct, conf, pred_cls, target_cls))
    ...
    # Plots
    if plots:
        confusion_matrix.plot(save_dir=save_dir, names=[names[k] for k in range(nc)], prefix='box_')
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    return (*box_res, *grid_res, *(loss.cpu() / len(dataloader)).tolist()), maps, t
